...
public object ShortenReferences {
    ...
    public fun process(elements: Iterable<JetElement>) {
        ...
        val shortenVisitor = ShortenReferencesVisitor(file, resolveAllVisitor.result)
        processElements(elements, shortenVisitor)
        shortenVisitor.finish()
    }
    ...
    private class ResolveAllReferencesVisitor(val file: JetFile) : JetVisitorVoid() {
    ...
    private class ShortenReferencesVisitor(val file: JetFile, val resolveMap: Map<JetReferenceExpression, BindingContext>) : JetVisitorVoid() {
        ...
        private fun bindingContext(expression: JetReferenceExpression): BindingContext = resolveMap[expression] ?: resolveSession.resolveToElement(expression)
