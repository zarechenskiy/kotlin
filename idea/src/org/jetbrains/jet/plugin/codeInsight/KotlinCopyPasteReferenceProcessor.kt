...
public class KotlinCopyPasteReferenceProcessor() : CopyPastePostProcessor<ReferenceTransferableData?> {
    ...
    private fun collectReferenceDataFromElement(
        ...
        for ((referencedDescriptor, declarations) in resolveMap) {
            ...
            val referenceData = collectReferenceData(file, startOffset, startOffsets, endOffsets,
                                                     declarations.first(), referencedDescriptor, element)
            if (referenceData != null) {
                collectedData.add(referenceData)
            }
        }
    ...
    private fun collectReferenceData(
        ...
        if (isExpressionWithReceiver && !referencedDescriptor.hasReceiver) {
            return null
        }
        val fqName = referencedDescriptor.fqName
        if (fqName != null && referencedDescriptor.canBeReferencedViaImport()) {
            return createReferenceData(referenceElement, startOffset, fqName)
        }
        return null
    }

    private fun DeclarationDescriptor.canBeReferencedViaImport(): Boolean {
        if (DescriptorUtils.isTopLevelDeclaration(this)) {
            return true
        }
    ...
    private fun createReferenceToRestoreData(expression: JetElement, originalReferencedFqName: FqName): ReferenceToRestoreData? {
        ...
        val referencedFqNames = referencedDescriptors.filterNot { ErrorUtils.isError(it) } .map { it.fqName }
        ...
        when {
            ...
            conflict -> {
                val mustBeReferencedWithReceiver = referencedDescriptors.any { it.hasReceiver }
...
private val DeclarationDescriptor.fqName: FqName?
    get() {
        if (this is ConstructorDescriptor) return getContainingDeclaration().fqName
...
private val DeclarationDescriptor.hasReceiver: Boolean
