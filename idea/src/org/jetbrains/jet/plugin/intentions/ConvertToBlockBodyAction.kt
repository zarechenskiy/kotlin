...
public class ConvertToBlockBodyAction : PsiElementBaseIntentionAction() {
    ...
    override fun invoke(project: Project, editor: Editor, element: PsiElement) {
        ...
        if (declaration is JetNamedFunction) {
            ...
            val needReturn = !KotlinBuiltIns.getInstance().isUnit(returnType) && !KotlinBuiltIns.getInstance().isNothing(returnType)
            val oldBodyText = body.getText()!!
            val newBodyText = if (needReturn) "return ${oldBodyText}" else oldBodyText
            val newBody = JetPsiFactory.createFunctionBody(project, newBodyText)
            ...
            fail("Unknown declaration type: $declaration")
        }
    }
