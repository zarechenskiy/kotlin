...
class KotlinBlock(
    ...
    private fun getChildrenAlignmentStrategy(): NodeAlignmentStrategy {
        ...
        return when (parentType) {
            VALUE_PARAMETER_LIST -> getAlignmentForChildInParenthesis(
            ...
            VALUE_ARGUMENT_LIST -> getAlignmentForChildInParenthesis(
                    jetCommonSettings.ALIGN_MULTILINE_PARAMETERS_IN_CALLS, VALUE_ARGUMENT, COMMA,
                    jetCommonSettings.ALIGN_MULTILINE_METHOD_BRACKETS, LPAR, RPAR)

            WHEN -> getAlignmentForCaseBranch(jetSettings.ALIGN_IN_COLUMNS_CASE_BRANCH)

            WHEN_ENTRY -> myAlignmentStrategy

            in BINARY_EXPRESSIONS -> {
                if (getOperationType(node) in ALIGN_FOR_BINARY_OPERATIONS)
                    NodeAlignmentStrategy.fromTypes(AlignmentStrategy.wrap(
                            createAlignment(jetCommonSettings.ALIGN_MULTILINE_BINARY_OPERATION, alignment)))
                else
                    NodeAlignmentStrategy.getNullStrategy()
            }

            SUPER_TYPE_LIST, INITIALIZER_LIST -> NodeAlignmentStrategy.fromTypes(AlignmentStrategy.wrap(
                    createAlignment(jetCommonSettings.ALIGN_MULTILINE_EXTENDS_LIST, alignment)))

            PARENTHESIZED -> object : NodeAlignmentStrategy() {
                val bracketsAlignment = if (jetCommonSettings.ALIGN_MULTILINE_BINARY_OPERATION) Alignment.createAlignment() else null
            ...
            else -> NodeAlignmentStrategy.getNullStrategy()
        }
    }
}
