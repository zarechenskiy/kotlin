...
fun Pseudocode.traverse(
        ...
        instructionAnalyzeStrategy: (Instruction)->Unit
) {
    val instructions = getInstructions(traversalOrder)
    for (instruction in instructions) {
        if (instruction is LocalFunctionDeclarationInstruction) {
            instruction.getBody().traverse(traversalOrder, instructionAnalyzeStrategy)
        }
        instructionAnalyzeStrategy(instruction)
    }
}
...
fun traverseFollowingInstructions(
    ...
    //TODO (why newArrayDeque is unresolved?)
    //val stack1 = Queues.newArrayDeque<Instruction>()
