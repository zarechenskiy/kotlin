...
public class KotlinTypeMapper {
    ...
    public Type mapClass(@NotNull ClassifierDescriptor classifier) {
        return mapClass(classifier.getDefaultType());
    }

    public Type mapClass(@NotNull KotlinType type) {
        return mapType(type, null, TypeMappingMode.CLASS_TYPE);
    }
    ...
    public Type mapType(
        ...
        return mapType(kotlinType, signatureVisitor, mode, null);
    }
    ...
    private Type mapType(
            ...
            @Nullable final JvmSignatureWriter signatureVisitor,
            @NotNull TypeMappingMode mode,
            @Nullable final TypeParameterMappings typeParameterMappings
    ) {


        Function1<String, KotlinType> mappings;
        if (typeParameterMappings == null) {
            mappings = null;
        } else {
            mappings = new Function1<String, KotlinType>() {
                @Override
                public KotlinType invoke(String s) {
                    TypeParameterMapping mapping = typeParameterMappings.get(s);
                    if (mapping != null) {
    ...
    public CallableMethod mapToAnyfiedCallableMethod(@NotNull FunctionDescriptor functionDescriptor, @NotNull ClassDescriptor classDescriptor) {
        ...
        return new CallableMethod(
                ...
                mapType(classDescriptor), null, null);
    }
    ...
    public JvmMethodGenericSignature mapSignatureWithGeneric(@NotNull FunctionDescriptor f, @NotNull OwnerKind kind) {
        return mapSignature(f, kind, false, null);
    }
    ...
    public JvmMethodGenericSignature mapSignature(
        ...
        if (f instanceof ClassConstructorDescriptor) {
            ...
            sw.writeReturnType();
            mapReturnType(f, sw); // TODO: map return type here
            sw.writeReturnTypeEnd();
        }
